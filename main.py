...
def main(VIDEO_STREAM=None, frame_queue: queue.Queue = None):  # Accept an optional override AND frame_queue
    os.makedirs(FULL_FRAME_ALERT_DIR, exist_ok=True)
    os.makedirs(OBJECT_IMAGE_DIR, exist_ok=True)

    model = YOLO(MODEL_PATH)
    item_class_ids = [
        k for k, v in model.model.names.items() if v in ITEM_CLASS_NAMES
    ]
    ...
    if not cap.isOpened():
        ...
        if frame_queue is not None:
            frame_queue.put(None) # Signal recorder to stop
        return

    while True:
        ret, frame = cap.read()
        if not ret:
            print("[INFO main.py] Video stream ended.")
            if frame_queue is not None:
                frame_queue.put(None)  # Signal end to recorder
            break

        # Create a clean copy of the frame for cropping and annotation later
        original_frame = frame.copy()
        
        # ✅ NEW BLOCK: Put the original frame into the queue for the recorder
        if frame_queue is not None:
            try:
                # Use nowait to avoid blocking the detection thread
                frame_queue.put_nowait(original_frame)
            except queue.Full:
                # This means the recorder is lagging behind.
                print("[WARN main.py] Frame queue is full. Recorder is lagging.")
        ...
        mask = np.array(
            [cls in item_class_ids + [PERSON_CLASS_ID] for cls in detections.class_id]
        )
        ...
        for tracker_id, class_id in zip(
            items_detections.tracker_id, items_detections.class_id
        ):
        ...
        person_box_map = {
            tid: box
            for tid, box in zip(
                persons_detections.tracker_id, persons_detections.xyxy
            )
        }
        item_box_map = {
            tid: box
            for tid, box in zip(items_detections.tracker_id, items_detections.xyxy)
        }
        ...
        for person_id, person_box in person_box_map.items():
            ...
            person_center = sv.Point(
                x=(person_box[0] + person_box[2]) / 2,
                y=(person_box[1] + person_box[3]) / 2,
            )

            for item_id, item_box in item_box_map.items():
                item_center = sv.Point(
                    x=(item_box[0] + item_box[2]) / 2,
                    y=(item_box[1] + item_box[3]) / 2,
                )
        ...
        for person_id in list(person_states.keys()):
            if person_id not in current_tracked_person_ids:
                ...
                person_last_box_original_scale = person_states[person_id].get(
                    "last_box"
                )
                ...
                if items_left and person_last_box_original_scale is not None:
                    for item_id in items_left:
                        if (
                            item_id in current_tracked_item_ids
                            and item_id not in orphan_candidates
                        ):
        ...
        for item_id in list(orphan_candidates.keys()):
            if item_id in current_tracked_item_ids:
                ...
                if time_since_orphaned > ALERT_PATIENCE_SECONDS:
                    ...
                    person_last_box_original_scale = orphan_data[
                        "person_last_box_original_scale"
                    ]
                    item_box_current_resized = item_box_map.get(
                        item_id
                    )  # Get item's *current* box (resized scale)
                    ...
                    if item_box_current_resized is not None:
                        try:
                            ...
                            x1_item, y1_item, x2_item, y2_item = map(
                                int, item_box_current_resized
                            )
                            x1_item, y1_item = (
                                int(x1_item / RESIZE_FACTOR),
                                int(y1_item / RESIZE_FACTOR),
                            )
                            x2_item, y2_item = (
                                int(x2_item / RESIZE_FACTOR),
                                int(y2_item / RESIZE_FACTOR),
                            )

                            # Ensure coordinates are valid
                            x1_item, y1_item = max(0, x1_item), max(0, y1_item)
                            x2_item, y2_item = (
                                min(original_frame.shape[1], x2_item),
                                min(original_frame.shape[0], y2_item),
                            )

                            cropped_item = original_frame[
                                y1_item:y2_item, x1_item:x2_item
                            ]

                            object_filename = f"{person_id}_{timestamp_str}_object_id_{item_id}.jpg"
                            object_save_path = os.path.join(
                                OBJECT_IMAGE_DIR, object_filename
                            )
                    ...
                    if person_last_box_original_scale is not None:
                        try:
                            x1_p, y1_p, x2_p, y2_p = map(
                                int, person_last_box_original_scale
                            )
                            cv2.rectangle(
                                annotated_alert_frame,
                                (x1_p, y1_p),
                                (x2_p, y2_p),
                                (0, 255, 0),
                                2,
                    ...
                    if item_box_current_resized is not None:
                        try:
                            ...
                            cv2.rectangle(
                                annotated_alert_frame,
                                (x1_item_orig, y1_item_orig),
                                (x2_item_orig, y2_item_orig),
                                (0, 0, 255),
                                2,
                            )  # Red for item
                            cv2.putText(
                                annotated_alert_frame,
                                f"Abandoned {item_name} (ID: {item_id})",
                                (x1_item_orig, y1_item_orig - 10),
                                cv2.FONT_HERSHEY_SIMPLEX,
                                0.7,
                                (0, 0, 255),
                                2,
                    ...
                    full_frame_filename = (
                        f"alert_P{person_id}_I{item_id}_{timestamp_str}.jpg"
                    )
                    full_frame_save_path = os.path.join(
                        FULL_FRAME_ALERT_DIR, full_frame_filename
                    )
        ...
        labels = [
            ...
            for tracker_id, class_id in zip(
                tracked_detections.tracker_id, tracked_detections.class_id
            )
        ]
        ...
        if cv2.waitKey(1) & 0xFF == ord("q"):
            if frame_queue is not None:
                frame_queue.put(None) # Signal end on manual quit
            break

    cap.release()
    cv2.destroyAllWindows()
    if frame_queue is not None:
        frame_queue.put(None) # Ensure recorder stops if loop breaks


if __name__ == "__main__":
    # This block will not be run when server.py imports it,
    # but it allows you to test main.py on its own if you want.
    print("[INFO] Running main.py standalone for testing...")
    main(VIDEO_STREAM=VIDEO_STREAM_URL, frame_queue=None) # Test with webcam 0